...
DEFAULT_COUNTRIES = {
    ...
    "Iranian exports": ["China", "UAE", "Syria", "Venezuela", "Malaysia", "Singapore"],
    "Iraqi exports": ["India", "China", "Turkey", "USA", "Italy", "Greece"],
}
...
def compute_yoy(root: ET.Element, countries: list, target_year: int, target_month: int):
    ...
    for m in root.findall(".//movement"):
        try:
            ...
            tonnes = float(m.findtext("qty_tonnes") or 0)
            date = datetime.strptime(m.findtext("load_port_date"), "%Y-%m-%d")
            if country in countries and date.month == target_month:
                if date.year == target_year - 1:
                    yoy_data[country]["prev"] += tonnes / 1000.0
                elif date.year == target_year:
                    yoy_data[country]["curr"] += tonnes / 1000.0
        except Exception:
            continue
    out = []
    tot_prev = tot_curr = 0.0
    for c in countries:
        p = yoy_data[c]["prev"]
        cval = yoy_data[c]["curr"]
        tot_prev += p
        tot_curr += cval
        change = "N/A" if p == 0 else round(((cval - p)/p)*100, 1)
        out.append([c, round(p,2), round(cval,2), change])
    total_change = "N/A" if tot_prev == 0 else round(((tot_curr - tot_prev)/tot_prev)*100, 1)
    out.append(["Total", round(tot_prev,2), round(tot_curr,2), total_change])
    return pd.DataFrame(out, columns=["Country", f"{target_year-1} (kt)", f"{target_year} (kt)", "YoY Change (%)"])
...
def fig_monthly(df: pd.DataFrame, countries: list, title: str):
    ...
    if df.empty:
        ...
        return fig
    df[countries].plot(kind="bar", stacked=True, ax=ax)
