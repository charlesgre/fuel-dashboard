...
def _read_bunker_excel(excel_path: str) -> pd.DataFrame:
    ...
    if 'DESCRIPTION' in df.columns:
        ...
        df['DESCRIPTION'] = df['SYMBOL']  # fallback

    # Date tolérante aux formats mixtes
...
def plot_bunker_price_diffs():
    ...
    df = _read_bunker_excel(excel_path)

    # --- Couples à tracer (incluant tes ajouts) ---
    ...
    required_symbols: Set[str] = {s for a, b in quote_pairs for s in (a, b)}
    df_filtered = df[df['SYMBOL'].isin(required_symbols)].copy()

    # Pivot tolérant aux doublons de date (plusieurs quotes le même jour → moyenne)
    ...
    # Ajustements spécifiques (ex: conversion PUAFZ00)
    ...
    # Diagnostics : couples manquants
    missing_pairs = [(a, b) for (a, b) in quote_pairs if a not in pivot.columns or b not in pivot.columns]
    if missing_pairs:
        st.warning(
            "Couples ignorés (symboles absents dans les données après nettoyage) : "
    ...
    for sym1, sym2 in quote_pairs:
        ...
        if diff_series.dropna().empty:
            st.info(f"Aucune donnée exploitable pour {sym1}-{sym2} (pas de dates communes / tout NaN).")
            continue
        ...
        # "FAKE_DATE" pour superposer les années sur un même axe (mois/jour)
        diff_df['FAKE_DATE'] = pd.to_datetime(
            dict(year=2000, month=diff_df['MONTH'], day=diff_df['DAY']),
            errors='coerce'
        )

        # Interpolation par année pour lisser les manques
        ...
        years_to_plot = sorted(diff_df['YEAR'].dropna().unique())
        ...
        for year in years_to_plot:
            ...
            if year_data['FAKE_DATE'].notna().sum() == 0:
                continue
        ...
        # Annotation du dernier point 2025 (si présent)
...
# Optionnel : exécuter directement dans Streamlit
if __name__ == "__main__":
    st.set_page_config(page_title="Bunker Price Diffs", layout="wide")
    st.title("Bunker Price Differences")
